...
Input:
1) Path of a folder containing multiple tiff files of a time series.
...
Input:
1) Path of a tiff file. The tiff file corresponds to a stack showing the photon counts associated with the GABA during an experiment.
...
Outputs:
1) Graph showing the total number of counts in each pixel of the original data. (figure : Image)

2) Graph showing the data with new pixel sizes. (figure with two panels : Pixel_reduction)

3) Graph showing which pixels were used to fit. The yellow pixels have a value of one and were used and the other pixels have a value of 0 and were not used. (figure : Fited_pixels)

4) Graph showing the calculated lifetime per pixel for the data with new pixel sizes. (Fited_pixels_tau)

5) Graph showing the monoexponential and biexponential fits on all of the photon counts. (Curve_fit_all_counts)

6) Graph showing all the curve fits done on pixels of the data with new pixel sizes. (Fited_pixels_all_curves)

7) Graph showing photon counts and curve fits done on pixels near the GABA puff and far from the GABA puff. (Fit_near_and_far_from_puff)  
...
Output:
1) HDF5 dataset saved in the dataset folder.


### data_reader_animation_mega_sim.py
Input:
1) Simulation results (HDF5 dataset).
...
Output:
1) The chosen animations are shown and saved (if you choose to).
...
3) The range of values for 'U_kcc2' and 'U_nkcc1' parameters (two tuples -> 'range_kcc2' and 'range_nkcc1')
...
Outputs:
1) Grid of the chloride intracellular concentrations before the GABA puff for all the simulations (figure : Stable chloride)

2) Grid of the potassium intracellular concentrations before the GABA puff for all the simulations (figure : Stable potassium)

3) Grid of the sodium intracellular concentrations before the GABA puff for all the simulations (figure : Stable sodium)

4) Grid of the membrane potential before the GABA puff for all the simulations (figure : Stable mp)

5) Grid of the chloride reversal potential before the GABA puff for all the simulations (figure : Stable Ecl)

6) Grid of the maximum chloride intracellular concentration variation due to the GABA puff for all the simulations (figure : Delta chloride)

7) Chloride concentration in soma as a function of time for all simulations (figure : Chloride steady state)

8) Potassium concentration in soma as a function of time for all simulations (figure : Potassium steady state)

9) Sodium concentration in soma as a function of time for all simulations (figure : Sodium steady state)

10) Membrane potential in soma as a function of time for all simulations (figure : Resting mp)

11) All graphs are saved as pickle file in the input folder.

### data_reader_pickle.py
Input:
1) Path of the folder containing pickle files from a run of 'kcc2_nkcc1_grid_analysis.py'.

Open the graphs in the folder. This makes it possible to edit them if you want to change something.

Output: 
1) See Output 1 to 10 in 'kcc2_nkcc1_grid_analysis.py'.